...
def add_cod(db, T, deal, deal_acc, cod, use_try=None):
    ...
    if deal_acc.gift and len(deal_acc.gift)> G_MIN_LEN:
        ...
        mess = '%s %s' % (mess, T('Остаток подарка: %s c разовой выплатой в %s руб.') % (float(deal_acc.gift_amount or 0), float(deal_acc.gift_pick or 0)))
        return mess
    ...
    if not partner_acc:
        ...
        if use_try:
            ...
            mess = T('Такого подарочного кода [%s] не существует. У Вас осталлось %s попыток.') % (cod, 6-deal_acc.gift_try)
        else:
...
def add_amo(db, T, deal, deal_acc, cod):
    ...
    if not gift_rec:
        return silent or T('Сейчас подарков для этого дела [%s] нет, но они могут появиться позже и будут автоматичеески начислены на этот подарочный код.') % deal_name
    ...
    mess = silent or '%s %s' % (mess, T('Сумма подарка: %s c разовой выплатой в %s руб.') % (float(amo_gift), float(gift_rec.pick)))
    mess = silent or '%s %s' % (mess, T('Эта разовая выплата будет добавляться как подарок к каждому вашему платежу по аккаунту [%s] для дела [%s] пока вся сумма подарка не израсходуется.') % (acc, deal_name))
...
def pick_ver(gift_amount, gift_pick):
    ...
    if vvv > 1: vvv=1
    elif vvv<PICK_GIFT_VERO_MIN: vvv=PICK_GIFT_VERO_MIN
    return vvv
...
def gift_proc(db, T, deal, deal_acc, request, session, GIFT_CODE):
    ...
    if GIFT_CODE:
        ...
        add_cod(db, None, deal, deal_acc, GIFT_CODE)
        add_amo(db, None, deal, deal_acc, GIFT_CODE)
