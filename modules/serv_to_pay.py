...
def make_edealer_payment(db, geted_pays, curr_in, xcurr, curr_out, ecurr, vol_in, volume_out, deal_acc_addr, rate,
                         order_stack_id=None, dealer=None, dealer_acc=None, dealer_deal=None):
    ...
    if max_bal and max_bal > 0 and max_bal - bal_dep < 0:  ##vol_in: тут уже в баланс упал вход
        mark_pay_ins(db, geted_pays, 'wait',
                     current.T('Stock overflow on the service: %s + %s > %s') % (bal_dep, vol_in, max_bal))
        db.commit()
        return
    ...
    if client:
        ...
        if ecurr:
            ...
            if dealer_acc and dealer_acc.balance > volume_out_full:
                ...
                dealer, dealer_acc, dealer_deal = ed_common.select_ed_acc(db, deal, ecurr, volume_out_full,
                                                                          unlim=USE_UNLIM)
                if dealer_acc:
                    print 'select_ed_acc -> dealer_acc:', dealer_acc.acc, dealer_acc.balance
                else:
                    mark_pay_ins(db, geted_pays, 'try',
                                 (dealer and dealer.name or 'dealer=None') + ': dealer_acc not found')
                    db.commit()
            ...
            if over_turns > 5:
                ...
                volume_out_full = volume_out_full * Decimal(
                    0.995)  # если превышение боле чем в 2 раза то 0.5% себе сразу
            elif over_turns > 2:
                volume_out_full = volume_out_full * Decimal(
                    0.9925)  # если превышение боле чем в 1 раза то 0.25% себе сразу
        ...
        if volume_out_full > balance:
            if len(geted_pays) == 1:
                ...
                for id in geted_pays:
                    ...
                    make_edealer_payment(db, [id], curr_in, xcurr, curr_out, ecurr, vol_in_one, volume_out_one,
        ...
        try:
            ...
            print 'deal_acc.to_pay = volume_out_full - amo_taken:', type(deal_acc.to_pay), type(volume_out_full), type(
                amo_taken)
...
def make_edealer_free_payment(db,
    ...
    volume_in = amo
    # log( db, 'make free-pay for acc: %s %s[%s]' % ( r.acc, volume_in, curr_in.abbrev))
    ...
    if client:
        ...
        try:
            # тут берем без учета что еще на нас начислит диллер за эту услугу (учет в ed_common.pay)
            volume_out, mess = db_client.calc_fees(db, deal, dealer_deal_NONE, curr_in, curr_out, amo, best_rate,
                                                   is_order, note=0)
    ...
    make_edealer_payment(db, geted_pays, curr_in, xcurr, curr_out, ecurr, volume_in, volume_out, deal_acc_addr,
...
def proc_xcurr(db, curr_in, xcurr):
    ...
    for r in db((db.orders_stack.ref_ == db.orders.id)
        ...
        make_edealer_payment(db, geted_pays, curr_in, xcurr, curr_out, ecurr, amo, volume_out, r.deal_acc_addrs, rate,
                             r.orders_stack.id, dealer=None, dealer_acc=None, dealer_deal=None)
