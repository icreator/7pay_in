    ...
    if dlr_deal.grab_form:
        ...
        for p in pay_pars:
            ...
            if calc != None and not (type(calc) in [type(dict()), type({})]):
                ...
                if calc == 'lm':
                    val = common.last_month()
                elif calc == 'lm2':
                    val = common.last_month2()
                elif calc == 'ly':
                    val = common.last_year()
                elif calc == 'ly2':
                    val = common.last_year2()
...
def calc_contract_amount(res, db, deal, edlr_deal, api_pars, edlr_acc, acc, amo, acc_name, testMake, log_on, pars_in):
    ...
    if contract_amount:
        ...
        if pars_in:
            ...
            return {'status': 'error',
                    'error': 'contract_amount = %s and pars_in: %s --> RES: %s' % (contract_amount, pars_in, res)}
...
def pay(db, deal, edlr, edlr_acc, edlr_deal, acc, amo, testMake=None, testConfirm=None, log_on=None, pars_in=None):
    ...
    # если там идет конвертация а сумма выражена не в рублях
    # то пересчитаем по курсу ЯД
    res = calc_contract_amount(res, db, deal, edlr_deal, api_pars, edlr_acc, acc, amo, acc_name, testMake, log_on,
                               pars_in)
    ...
    if 'credit_amount' in res:
        ...
        # print 'TAX:', tax
        if tax < 0:
            tax = 0  # это тестовый платеж где баланс не меняется
        elif tax > 9:
            tax = 1  # есл вдруг было какоето изменение на счету не от этого платежа
        res['tax'] = tax
        res['sum_taken'] = sum1  # списанные деньги
...
def YmTo(db, deal, edlr_deal, api_pars, token, acc, amount, acc_name, test=None, log_on=None, pars_in=None):
    if edlr_deal.scid == 'phone-topup':
        if False and len(acc) == 11 and acc[0:1] == '7':
            ph = acc[1:]
        else:
            ph = acc
        # print ph
        ...
        if type(deal) == type(''):
            ...
            name = deal.name  # .decode('utf8') #'cp1251')
        # print name
        pars = {'pattern_id': edlr_deal.scid,
