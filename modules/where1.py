...
def found_buys(db, addr):
    ...
    for r in db(
        ...
        mess_in = '%s [%s] - op_id:%s %s' % (rec.amount, 'RUB', no_addr and '' or rec.operation_id,
                                             no_addr and rec.amo_out and round(rec.amount / rec.amo_out, 8) or '')
        if rec.xcurr_id:
        ...
        mess_out = rec.amo_out and '%s [%s] %s - txid:%s' % (
        rec.amo_out, abbrev, rec.created_on, rec.txid) or '0[%s] %s' % (abbrev, st)
        res.append({
            T('Вход'): mess_in,
            T('Выход'): mess_out,
...
def found_pay_ins(db, curr_in, xcurr_in, addr, pays, amo_rest):
    ...
    for pay in db(
        ...
        if pay.pay_ins.order_id:
            ...
            order.id, order.created_on, round(order.volume_out / order.volume_in, 8))
        st = pay.pay_ins.status
        if st == 'returned':
            pay_out_info = T('Возвращен обратно')
        else:
        ...
        if pay.pay_ins.payout_id:
            ...
            if payout and payout.dealer_acc_id:
                ...
                dealer_deal = db(
                    (db.dealer_deals.deal_id == deal.id) & (db.dealer_deals.dealer_id == dealer.id)).select().first()
                ...
                if not privat:
                    if 'payment_id' in p_i:
                        ...
                        p_i['payment_id'], p_i.get('invoice_id') or p_i.get('payee'))
                    else:
        ...
        if payout:
            ...
            if payout.amo_partner and payout.amo_partner > 0:
                rec_vals[T('Партнёрские')] = T('Вы получили дополнительно %s [%s]') % (
