...
def index():
    ...
    for curr in db(db.currs).select():
        input_currs.append(
            (
            CAT(IMG(_src=URL('static', 'images/currs/' + curr.abbrev + '.png'), _width=30, _alt=''),
                        ' ', curr.name), False, URL()
                  )
            )
    
    input_curr_selected = db.currs[1]
    input_currs = [
        (CAT(IMG(_src=URL('static', 'images/currs/' + input_curr_selected.abbrev + '.png'), _width=30, _alt=''), ' ', input_curr_selected.name),
            False, None, input_currs)
        ]


    return dict(title=title, subtitle=subtitle, inp_currs=inp_currs,
               abbrev=abbrev, addr=addr, vol=vol, input_currs = input_currs)
